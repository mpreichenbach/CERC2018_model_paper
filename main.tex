...
\section{Modeling Framework}
	...
	\subsection{Covariates}
		...
		\subsubsection{Numerical Covariates} \label{sec:num-cov}
			...
			Figure \ref{img:variograms} shows the variograms with fitted models for each Pond/Trial combination, and Figure \ref{img:intensities} shows interpolated values in each pond.
			
			\begin{figure}
				%\includegraphics[width=\textwidth]{variograms.png}
				\caption{Variograms and fitted models.}
				\label{img:variograms}
			\end{figure}
...
\section{Results}
	...
	\subsection{State-Switching Probabilities}
	
		To determine whether the covariate \emph{dB} had a common effect on carp behavior, we investigated plots of the state-switching probabilities $\lambda_{i \to j}$, with $i, j \in \{1, 2\}$, as a function of dB (re 1 $\mu$Pa).
