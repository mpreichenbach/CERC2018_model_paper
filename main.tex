...
\section{Results}
	...
	\subsection{Top Models by AIC Score}
		...
		Table \ref{tbl:cov_freq} gives the frequency which the five covariates \emph{Trial, Temp, dB, Diel} and \emph{Trt} appear in the top models for each time frame.
		
		\begin{table}[H]
			\centering
			\begin{tabular}{|c|c|c|c|c|}
				\hline
				\thead{Covariate} & \makecell{\thead{1min} \\ Frequency} & \makecell{\thead{5min \\ Frequency}} & \makecell{\thead{30min \\ Frequency}} & \thead{Total} \\
				\hline
				\emph{Trial} & & & & \\
				\emph{Temp} & & & & \\
				\emph{Diel} & & & & \\
